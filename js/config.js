...
const ShopUtils = {
    ...
    getProductsByMaterial: function(material) {
        return SHOP_PRODUCTS.filter(product => product.material === material);
    },
    
    getAllMaterials: function() {
        const materials = SHOP_PRODUCTS.map(product => product.material);
        return [...new Set(materials)];
    },
    
    formatPrice: function(price) {
        return `${price}${SHOP_CONFIG.currency}`;
    },
    
    getProductImagePath: function(product) {
        if (!product) return SHOP_CONFIG.defaultImage;
        return `${SHOP_CONFIG.imageBasePath}${product.sku}.jpg`;
    }
};
